...
La auditoría pública encontró una restricción real en el robots raíz (`Disallow: /`). Se corrigió con `Allow: /umbral-blender-web/` en el mismo grupo y se añadió el sitemap de UMBRAL, conservando las restricciones de las otras rutas. La respuesta pública y la reauditoría SEO del 21 de septiembre de 2026 a las 21:13 confirmaron la excepción: `robots-txt` e `is-crawlable` pasaron, con puntuación SEO 100. Las otras tres categorías se midieron en la corrida móvil completa anterior; los detalles y el alcance están en `docs/qa-report.md`. [Google: aplicación de la regla de ruta más específica](https://developers.google.com/crawling/docs/robots-txt/robots-txt-spec#order-of-precedence-for-rules).
...
Presupuestos internos de referencia: imagen principal hasta 300 KB, fuentes e iconos iniciales hasta 150 KB, JavaScript inicial hasta 100 KB comprimido. En la auditoría móvil pública se registraron 20.6 KB para la imagen principal, 58.4 KB para fuentes/iconos y 12.8 KB de JavaScript, incluyendo la transferencia registrada por Lighthouse. No se solicitó Three.js ni el GLB durante la navegación inicial. La experiencia y el PDF se cargan cuando se solicitan. El hero combina renders optimizados y composición CSS; reservar dimensiones, reducir texturas y detener el renderizado fuera de vista siguen siendo criterios para futuras iteraciones.
